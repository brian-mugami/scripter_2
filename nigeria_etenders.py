...
options = Options()
Title = "Nigeria Tenders(E Tenders Nigeria)"
options.add_argument('--headless')
driver = webdriver.Chrome(service=service, options=options)
et_url = "https://etenders.com.ng/"
timeout = 30
driver.get(et_url)
...
def get_table_data(keywords, num=1):
    table = WebDriverWait(driver, 30).until(
        EC.presence_of_element_located((By.XPATH, '//table[@id="ptp_6d055cfabedfdbcb_2"]')))
            ...
            try:
                next_button = WebDriverWait(driver, 30).until(
                    EC.element_to_be_clickable(
                        (By.XPATH,
                         '//div[@class="posts-table-below posts-table-controls"]//a[@id="ptp_6d055cfabedfdbcb_2_next"]')))
