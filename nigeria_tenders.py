...
options = Options()
Title = "Nigeria Tenders(Public Procurement)"
options.add_argument('--headless')
driver = webdriver.Chrome(service=service, options=options)
et_url = "https://publicprocurement.ng/tenders/"
timeout = 30
driver.get(et_url)
...
def get_table_data(keywords, num=1):
    ...
    for row in rows:
        ...
        row_dict["link"] = cells[1].find_element(by=By.XPATH, value=".//a").get_attribute("href")
        row_dict["page"] = num
        tender_name = row_dict.get("TENDER DESCRIPTION", "").lower()
